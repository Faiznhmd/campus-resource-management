...
function normalizeTime(str: string) {
  return str.replace(/:\d+/g, '').replace(/\s+/g, '').toLowerCase();
}
...
const ResourceDetailsPage = () => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      // ✅ SHOW CORRECT SUCCESS MESSAGE BASED ON approval requirement
      if (resource?.requiresApproval) {
        message.success(
          'Your booking request was submitted! Waiting for admin approval.'
        );
      } else {
  ...
  return (
    ...
    >
      ...
      {/* ✅ NEW SECTION — SHOW APPROVAL REQUIREMENT */}
      <p>
        <strong>Approval Required:</strong>{' '}
        <Tag color={resource.requiresApproval ? 'orange' : 'green'}>
          {resource.requiresApproval
            ? 'YES (Admin Approval Needed)'
            : 'NO (Instant Booking)'}
        </Tag>
      </p>

      <div style={{ marginTop: 20 }}>
        <h3>Select Booking Times</h3>
        ...
        {/* ✅ UPDATED BUTTON TEXT BASED ON requiresApproval */}
        <Button
          type="primary"
          disabled={!isAvailable}
          onClick={handleBooking}
